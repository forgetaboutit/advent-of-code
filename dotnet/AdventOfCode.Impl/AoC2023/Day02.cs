...
public static class Day02
{
    public static AoCResult<int, int> Solve()
    {
        ...
        var first = games
            .Where(
                static game => game.Sets.All(
                    static set => set.HasAtMost("red", 12)
                                  && set.HasAtMost("green", 13)
                                  && set.HasAtMost("blue", 14)))
            .Sum(static game => game.Id);

        var second = games
            .Select(static game => game.Power())
            .Sum();

        return AoCResult.Create(first, second);
    }

    private static Game ParseGame(
        string line)
    {
        ...
        var sets = parts[1]
            ...
            .Select(
                r => new Set(
                    r.Select(c => c.Split(" "))
                        .Select(c => new Cube(c[1], int.Parse(c[0])))
                        .ToDictionary(c => c.Color)))
            .ToList();
    ...
    {
        public int Power()
            => Sets.SelectMany(static set => set.Cubes)
                .GroupBy(static kvp => kvp.Key)
                .Select(
                    static byColor => (
                        byColor.Key,
                        SufficientCount: byColor.Max(static kvp => kvp.Value.Count)))
                .Aggregate(
                    1,
                    static (acc, pair) => pair.SufficientCount * acc);
    }
        ...
        Dictionary<string, Cube> Cubes)
    {
        public bool HasAtMost(
            string color,
            int number)
            => !Cubes.TryGetValue(color, out var cube)
               || cube.Count <= number;
    }
