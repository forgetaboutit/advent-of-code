...
public static class Day03
{
    ...
    {
        var (symbolCells, numberRanges) = ParseSymbolsAndNumbers(Input);

        var first = numberRanges
            .Where(
                rangeKvp => symbolCells.Any(
                    symKvp => rangeKvp.Key.IsAdjacentDiagonally(symKvp.Key)))
            .Select(static rangeKvp => rangeKvp.Value)
            .Sum();

        var second = symbolCells
            .Where(static symKvp => symKvp.Value == '*')
            .Select(
                symKvp => numberRanges.Where(
                        numKvp => numKvp.Key.IsAdjacentDiagonally(symKvp.Key))
                    .ToArray())
            .Select(
                static adjacentNumbers => adjacentNumbers is [var fst, var snd]
                    ? fst.Value * snd.Value
                    : 0)
            .Sum();
    ...
    private static
        (IReadOnlyDictionary<Position2D, char> symbolCells,
        IReadOnlyDictionary<Range2D, int> numberRanges)
        ParseSymbolsAndNumbers(
            string input)
    {
        var symbolCells = new Dictionary<Position2D, char>();
        var numberRanges = new Dictionary<Range2D, int>();
        var y = 0;

        foreach (var line in input.SplitWithNewline())
        {
            var x = 0;
            int? numberStartIndex = null;
            int? numberEndIndex = null;

            foreach (var c in line)
            {
                if (IsSymbol(c))
                {
                    symbolCells.Add(new Position2D(x, y), c);
                }

                if (char.IsDigit(c)
                    && numberStartIndex is null)
                {
                    numberStartIndex = x;
                    numberEndIndex = x;
                }
                else if (char.IsDigit(c)
                         && numberStartIndex is not null)
                {
                    numberEndIndex += 1;
                }
                else if (!char.IsDigit(c)
                         && (numberStartIndex, numberEndIndex) is ({} s, {} e))
                {
                    numberRanges.Add(
                        new Range2D(
                            new Position2D(s, y),
                            new Position2D(e, y)),
                        int.Parse(line[s..(e + 1)]));
            ...
            // Handle case where line ends with a digit and therefore numberEndIndex
            // is not set yet.
            if (numberStartIndex is {} si)
            {
                var lastIndex = x - 1;
        ...
        return (symbolCells, numberRanges);
    }
