...
public static class Day03
{
    ...
    {
        var first = FollowInstructions(Input, 1);
        var second = FollowInstructions(Input, 2);

        return AoCResult.Create(first, second);
    }

    private static int FollowInstructions(
        string directions,
        int numberOfPositions)
    {
        var visitedHouses = VisitHouses(
            directions,
            numberOfPositions);

        return CountHousesVisitedAtLeastOne(visitedHouses);
    }

    private static SparseGrid<int> VisitHouses(
        string directions,
        int numberOfPositions)
    {
        var visitedHouses = new SparseGrid<int>();
        var positions = new Position2D[numberOfPositions];
        var positionIndex = 0;
        ...
        {
            ref var currentPosition = ref positions[positionIndex];
            ...
            visitedHouses.AddOrUpdate(
                currentPosition,
                1,
                static cell => cell.Value + 1);
    ...
    private static int CountHousesVisitedAtLeastOne(
        SparseGrid<int> grid)
        => grid
            .Count(static c => c.Value > 0);
